...
var request = require('request'),
  ...
  debug = require('debug')(require('../package.json').name + ':f1')
var jjv = new JJV()
...
F1.prototype.get_token = function(callback) {
  async.waterfall([
      if (this.config.userURL) {
        debug('Already authenticated, yet attempting to authenticate again...')
      }
      request.post.bind(request, this.config.apiURL + '/PortalUser/AccessToken', {
        ...
        if (res.statusCode === 200) {
          ...
          debug('Auth headers were: %j', res.headers)
